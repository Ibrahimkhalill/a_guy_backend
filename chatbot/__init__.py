...
class DialogueFSM:
    ...
    def _get_current_question(self) -> str:
        ...
        if not (0 <= self.current_question_index < len(questions)):
            logger.warning(
                f"Invalid question index: {self.current_question_index} for exercise with {len(questions)} questions.")
            return translate_text_to_english("No question available.")
            ...
            try:
                ...
                if svg_content:
                    ...
                    if self.current_svg_description:
                        translated_svg_desc = translate_text_to_english(
                            self.current_svg_description)
    ...
    def transition(self, user_input: str) -> str:
        ...
        if self.state == State.START:
            ...
            topics_str = ", ".join(
                available_topics[:5]) if available_topics else "Any topic"
        ...
        # --- End State ---
            ...
            if text_lower in {"hi", "hello", "hey"}:
                self.state = State.SMALL_TALK
                response_text = random.choice(I18N["small_talk_prompts"])
